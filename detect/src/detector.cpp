...
cv::Mat ArmorDetector::preprocess(const cv::Mat& frame)
{
    ...
    int targetW = input_size_.width, targetH = input_size_.height;

    float scale = std::min((float)targetW / w, (float)targetH / h);
    ...
    cv::Mat padded;
    cv::copyMakeBorder(resized, padded, top, padH - top, left, padW - left,
                                ...
                                std::vector<ArmorObject>& results)
{
    ...
    for (int i = 0; i < maxDet; ++i) {
        ...
        centers.push_back(cv::Point2f(bx + bw / 2.0f, by + bh / 2.0f));
    }
